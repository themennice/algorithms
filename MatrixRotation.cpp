...
void matrixRotation(vector<vector<int>> matrix, int r) {
    ...
    int lay = min(matrix.size(), matrix[0].size())/2;
    ...
    while (level<lay)
    {
        int tr=(2*(row+1-2*level)-2)+(2*(col+1-2*level)-2);
        q = r%tr;
    
        for(int f=0; f<q;f++)
        {
            for(int t = level; t < col - level; t++)
            {
                swap(matrix[level][t],matrix[level][t+1]);
            }
    
            for(int r = level; r < row - level; r++)
            {
                swap(matrix[r][col-level], matrix[r+1][col-level]);
            }
    
            for(int b = col - level; b > level; b--)
            {
                swap(matrix[row-level][b], matrix[row-level][b-1]);
            }
    
            for(int l = row - level; l > level+1; l--)
            {
                swap(matrix[l][level], matrix[l-1][level]);
            }
        }
        level++;
